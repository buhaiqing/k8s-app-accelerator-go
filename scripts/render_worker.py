...
class RenderWorker:
    ...
    def render(self, template_path: str, context: dict) -> str:
        ...
        try:
            ...
            self.logger.error(f"渲染模板失败 [{template_path}]: {str(e)}", exc_info=True)
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--worker-mode':
        ...
        # Worker 循环结束统计
        logger.info("=" * 60)
        logger.info(f"Worker 停止运行")
        logger.info(f"总处理请求数：{request_count}")
        logger.info(f"错误请求数：{error_count}")
        logger.info(f"成功率：{(request_count - error_count) / request_count * 100:.2f}%" if request_count > 0 else "无请求")
        logger.info("=" * 60)
        ...
        if len(sys.argv) < 3:
            print("Usage:")
            print("  Test mode: python3 render_worker.py <template_path> <context_json>")
